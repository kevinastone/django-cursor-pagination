...
class CursorParameter(object):
	...
	def filter_param(self):
		...
		if not self.unique:
			# If the parameter is not unique, we have to allow equivalent values
			operator = operator + "e"
		
		param = {
			"{field_name}__{operator}".format(field_name=self.field_name, operator=operator): self.value
		}
		return param
...
class BaseCursor(object):
	...
	def get_ordering(queryset):
		ordering = list(queryset.query.extra_order_by) + list(queryset.query.order_by)
		if ordering:
			return ordering
	...
	def from_queryset(cls, queryset, obj, ascending=True):
		...
		for field_name in ordering:
			...
			parameter = CursorParameter(field_name, getattr(obj, field_name), ascending=parameter_ascending, unique=field.unique)
